...
logger = logging.getLogger("dearbobby_foundit")

WORKSPACE_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
SESSION_FILE = os.path.join(WORKSPACE_ROOT, "foundit_session.json")
LOG_FILE = os.path.join(WORKSPACE_ROOT, "applied_log.csv")

SELECTORS = {
    "job_card": "div.srpResultCard, div.jobTupleCard, div.card-body",
    "job_title": "div.jobTitle a, h3.title a, a.title",
    "company_name": "div.companyName, span.company, a.company",
    "location": "div.location, span.loc",
    "posted_date": "div.postedDate, span.posted",
    "jd_container": "div.jobDescription, div.job-description-content",
    "apply_button": "button.apply-btn, button:has-text('Quick Apply'), button:has-text('Apply')"
}
...
def parse_freshness_to_days(posted_text: str) -> float:
    ...
    if "today" in txt or "just" in txt or "1 day" in txt:
        return 0.0
    import re
...
def handle_screening_questions(page: Page, profile: dict, resume_keywords: set):
    try:
        labels = page.locator("label, legend, div.question-text").all()
        ...
        logger.warning(f"Error handling Foundit screening questions: {e}")

def search_jobs(page: Page, title: str, location: str) -> list:
    url = f"https://www.foundit.in/srp/results?query={title}&locations={location}"
    logger.info(f"Navigating to Foundit India: {url}")
    page.goto(url, wait_until="domcontentloaded")
    page.wait_for_timeout(3000)
    
    jobs = []
    cards = page.locator(SELECTORS["job_card"]).all()
    logger.info(f"Found {len(cards)} job cards on Foundit page.")
...
def run(profile: dict, resume_keywords: set):
    logger.info("Initializing Playwright session for Foundit India...")
    ...
    with sync_playwright() as p:
        ...
        if os.path.exists(SESSION_FILE):
            try:
                ...
                logger.info("Loaded cached Foundit session storage state.")
            except Exception:
        ...
        try:
                ...
                for location in locations:
                    ...
                    for job in jobs:
                        ...
                        if not url or url in processed_urls:
                            if url:
                                logger.info(f"==> [SKIPPED - DUPLICATE] Title: '{job['title']}' | Company: '{job['company']}' (Foundit)")
                            continue
                            
                        logger.info(f"Evaluating job card: '{job['title']}' at '{job['company']}' (Foundit)")
                        ...
                        if (score_details["score"] < threshold and not is_skill_match_pass) or score_details.get("excluded"):
                            logger.info(f"==> [SKIPPED] Title: '{job['title']}' | Company: '{job['company']}' | Score: {score_details['score']}/100 (Foundit)")
                        ...
                        if not apply_btn.is_visible():
                            logger.info(f"==> [SKIPPED - EXTERNAL REDIRECT] Title: '{job['title']}' | Company: '{job['company']}' (Foundit)")
                            log_application(job, score_details, False)
                            processed_urls.add(url)
                            continue
                        ...
                        logger.info(f"==> [APPLIED #{applications_submitted}] Title: '{job['title']}' | Company: '{job['company']}' | Score: {score_details['score']}/100 (Foundit)")
            ...
            try:
                ...
                pass
            browser.close()
            logger.info("Foundit automation run completed.")
